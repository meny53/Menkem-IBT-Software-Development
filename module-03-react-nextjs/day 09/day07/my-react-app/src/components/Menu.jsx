...
// Profiler callback to record measurements for PROFILE.md
...
function Menu({ shouldCrashDishId = null }) {
  ...
  // Stable handler for adding to cart (avoids re-rendering all memoized dish cards)
  ...
  // Filtered dishes memoized to prevent re-filtering when unrelated state changes
      ...
      <section className="menu-section">
        <div className="menu-header-bar">
          ...
          <div className="menu-header-controls">
            ...
            {/* Deliberate error simulation control */}
            <button
              type="button"
              className="btn-test-crash menu-crash-toggle"
              onClick={() => setGlobalCrashSimulated(true)}
              title="Deliberately throw inside one dish to test boundary isolation"
            >
              💥 Crash Dish #1 (Test Boundary 1)
            </button>
          </div>
        </div>
        ...
        {filteredDishes.length === 0 ? (
          ...
          <div className="card-container">
            {filteredDishes.map((dish, index) => {
              // Trigger deliberate crash on the first dish if simulated or id matches
              const shouldCrash =
        ...
        {/* Modal rendered with createPortal, Escape handler & focus return */}
        <DishModal
          dish={selectedDish}
